...
public class Main {
	...
	public static void main(String[] args) throws IllegalAccessException, InvocationTargetException, InstantiationException, IllegalArgumentException,  SecurityException, NoSuchMethodException {
		
		List<String> randomWords = Arrays.asList("truck", null, "car", "phone", "list", null ,"school", null, "lecture");
		...
		List<String> objects1 = Arrays.asList(
				"Phone", "Wired phone", "Cell phone");
		List<String> objects2 = Arrays.asList(
				"Car", "Truck", "Boat");
		List<String> objects3 = Arrays.asList(
				"Monkey", "Elephant", "Lion");
		
		List<List<String>> allObjects = Arrays.asList(objects1, objects2, objects3);
		List<String> allObjectsCollection = allObjects.stream()
				.flatMap(x->x.stream())
		...
		List<Contacts> myContacts = Arrays.asList(
				new Contacts(new User("Mirian Surmanidze", 21, "Georgia", "Batumi"), CountryPhoneCodes.GEORGIA, 555100100),
				new Contacts(new User("Nika Shainidze", 25, "Georgia", "Batumi"), CountryPhoneCodes.GEORGIA, 577200200)
				);
		...
		Optional<String> newValue = values.stream().reduce( (value1, value2) -> { return value1+value2; } );
		...
		Camera cameraThree = new Camera("Dual 12MP", "5X", 3);
		  ...
		  LOGGER.info(phoneFour.hashCode()); LOGGER.info(phoneTwo.equals(phoneFour) +
		  "\n");
		  ...
		  userTwo.setUserAge(50); userTwo.setUserNationality("Georgia");
